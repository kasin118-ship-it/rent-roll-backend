...
[${b.name}]
  ...
  > Occupancy Rate: ${percent.toFixed(2)}%
      ...
      if (occupied > rentable) {
        console.log(
          '  WARNING: OVER-OCCUPIED! (Data inconsistency or double-booking)',
        );
    ...
    const grandPercent =
      grandTotalRentable > 0
        ? (grandTotalOccupied / grandTotalRentable) * 100
        : 0;
    console.log('--------------------------------------------------');
    console.log('GRAND TOTALS:');
    console.log(
      `  Rentable : ${grandTotalRentable.toLocaleString('en-US', { minimumFractionDigits: 2 })} sqm`,
    );
    console.log(
      `  Occupied : ${grandTotalOccupied.toLocaleString('en-US', { minimumFractionDigits: 2 })} sqm`,
    );
