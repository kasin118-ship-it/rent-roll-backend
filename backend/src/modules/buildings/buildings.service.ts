...
export class BuildingsService {
  ...
  async create(
    ...
    if (existing) {
      throw new ConflictException(
        'Building code already exists in this company',
      );
    }
  ...
  async findAll(companyId: string): Promise<any[]> {
    ...
    console.log(
      'Rented Areas by Building ID:',
      Object.fromEntries(rentedAreaMap),
    );
    buildings.forEach((b) => {
      console.log(
        `Building: ${b.name}, ID: ${b.id}, Total Area: ${b.rentableArea}, Rented: ${rentedAreaMap.get(b.id) || 0}`,
      );
    });
    console.log('----------------------------');
