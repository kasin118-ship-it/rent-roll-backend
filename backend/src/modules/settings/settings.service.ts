...
export class SettingsService {
    ...
    async getExportData(companyId: string) {
        ...
        const customers = await this.dataSource.manager.find(Customer, { where: { companyId } });
        const buildings = await this.dataSource.manager.find(Building, { where: { companyId } });
    ...
    async importData(companyId: string, data: any) {
        ...
        try {
            ...
            const { customers, buildings, contractMaster, rentalSpaces, pricingTiers } = data;

            // 1. Import Customers
            if (customers && customers.length > 0) {
                for (const row of customers) {
                    const existing = await queryRunner.manager.findOne(Customer, { where: { id: row.customer_id } });
                    if (!existing) {
            ...
            if (buildings && buildings.length > 0) {
                for (const row of buildings) {
                    const existing = await queryRunner.manager.findOne(Building, { where: { id: row.building_id } });
                    if (!existing) {
            ...
            if (contractMaster && contractMaster.length > 0) {
                for (const row of contractMaster) {
                    // Find customer
                    const customer = await queryRunner.manager.findOne(Customer, { where: { id: row.customer_id } });
                    if (!customer) continue; // Skip if customer not found

                    // Create Contract
                    let contract = await queryRunner.manager.findOne(RentContract, { where: { contractNo: row.contract_no } });
                    if (!contract) {
                    ...
                    const spaces = rentalSpaces.filter((s: any) => s.contract_no === row.contract_no);
                    for (const space of spaces) {
                        const building = await queryRunner.manager.findOne(Building, { where: { id: space.building_id } });
                        if (!building) continue;

                        let unit = await queryRunner.manager.save(ContractUnit, {
                        ...
                        const tiers = pricingTiers.filter((t: any) => t.contract_no === row.contract_no && t.floor.toString() === space.floor.toString());
            ...
            return { success: true, message: `Imported ${contractMaster?.length || 0} contracts` };
