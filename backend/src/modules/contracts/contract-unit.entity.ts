...
export class ContractUnit {
  ...
  // Relations
  @ManyToOne(() => RentContract, (contract) => contract.contractUnits, {
    onDelete: 'CASCADE',
  })
  ...
  @OneToMany(() => RentPeriod, (rentPeriod) => rentPeriod.contractUnit, {
    cascade: true,
  })
  ...
  get currentRent(): number {
    ...
    const now = new Date();
    const current = this.rentPeriods.find(
      (p) => new Date(p.startDate) <= now && new Date(p.endDate) >= now,
    );
    return current
      ? Number(current.rentAmount)
      : Number(this.rentPeriods[0].rentAmount);
  }
  ...
  get currentServiceFee(): number {
    ...
    return current
      ? Number(current.serviceFee)
      : Number(this.rentPeriods[0].serviceFee);
  }
}
