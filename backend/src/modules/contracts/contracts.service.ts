...
export class ContractsService {
    ...
    async create(createDto: CreateContractDto, userId: string, companyId: string, files?: Express.Multer.File[]): Promise<RentContract> {
            ...
            if (files && files.length > 0) {
                const documents = await Promise.all(
                    files.map(async (file) => {
                        const path = await this.gcsService.uploadFile(file, `contracts/${savedContract.id}`);
    ...
    async getExpiringContracts(companyId: string, days: number): Promise<RentContract[]> {
    ...
    private validateRentPeriods(
        periods: { startDate: string; endDate: string; rentAmount: number; serviceFee?: number }[],
        ...
        if (sorted[0].startDate !== contractStart) {
            throw new BadRequestException('First rent period must start at contract start date');
        }

        // Check last period ends at contract end
        if (sorted[sorted.length - 1].endDate !== contractEnd) {
            throw new BadRequestException('Last rent period must end at contract end date');
        }
        ...
        for (let i = 0; i < sorted.length - 1; i++) {
            ...
            if (current.endDate !== next.startDate) {
                throw new BadRequestException('Rent periods must be continuous without gaps or overlaps');
            }
        }
